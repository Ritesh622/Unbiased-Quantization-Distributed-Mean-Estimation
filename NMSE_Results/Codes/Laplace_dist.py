...
# Set the current directory and update the system path.
...
for n_index in range(len(num_users_list)):
    ...
    for instanceno in range(num_instances):
        ...
        #----- Generate Laplace Distributed Vectors -----
        # Each vector is drawn from a Laplace distribution with loc=1 and scale=2.
        for _ in range(i):
            vec = np.random.laplace(loc=1, scale=2, size=dim)
        ...
        # For each vector, compute the quantized output for each scheme and update the running average.
        for v in vec_list:
...
with open(os.path.join(files_dir, 'nmse_avg_data_Laplace_dist.pkl'), 'wb') as avg_file:
    pickle.dump(nmse_avg_data, avg_file)

with open(os.path.join(files_dir, 'nmse_max_data_Laplace_dist.pkl'), 'wb') as max_file:
