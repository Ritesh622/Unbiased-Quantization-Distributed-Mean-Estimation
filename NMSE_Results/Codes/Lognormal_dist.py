...
# Create tensors on the device to store NMSE results for various quantization schemes.
...
for n_index in range(len(num_users_list)):
    ...
    for instanceno in range(num_instances):
        ...
        #----- Generate Lognormal Distributed Vectors -----
        # Generate 'i' random vectors drawn from a lognormal distribution.
        # (Note: Although the paper uses (0,1), here we use mean=1 and sigma=2 as specified.)
        for _ in range(i):
            vec = np.random.lognormal(mean=1, sigma=2, size=dim)
...
with open(os.path.join(files_dir, 'nmse_avg_data_Lognormal_dist.pkl'), 'wb') as avg_file:
    pickle.dump(nmse_avg_data, avg_file)

with open(os.path.join(files_dir, 'nmse_max_data_Lognormal_dist.pkl'), 'wb') as max_file:
