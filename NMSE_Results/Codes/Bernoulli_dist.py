...
#============= Setup Current Directory and Path =============
current_dir = os.getcwd()
current_path = pathlib.Path(__file__).parent.resolve()
sys.path.insert(0, str(current_path) + "/../")
#======================

#============= Create Parent Folder for Results =============
# Create a parent folder "Distributions_NMSE_Results" to store result pickle files.
...
# Create tensors on the device to store NMSE values for each quantization scheme.
...
for n_index in range(len(num_users_list)):
    ...
    for instanceno in range(num_instances):
        ...
        # Generate 'i' random binary vectors (Bernoulli distribution with p=[0.3, 0.7]).
        for _ in range(i):
            vec = np.random.choice(np.arange(2), size=dim, p=[0.3, 0.7])
        ...
        NMSE_DRIVE_Hadamard_temp   += torch.norm(emp_mean_est_DRIVE_Hadamard - emp_mean).pow(2)   / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_EDEN_Hadamard_1bit_temp += torch.norm(emp_mean_est_EDEN_Hadamard_1bit - emp_mean).pow(2) / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_EDEN_Hadamard_2bit_temp += torch.norm(emp_mean_est_EDEN_Hadamard_2bit - emp_mean).pow(2) / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Type_Biased_1bit_temp   += torch.norm(emp_mean_Type_Biased_1bit - emp_mean).pow(2)   / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Type_Unbiased_1bit_temp += torch.norm(emp_mean_Type_Unbiased_1bit - emp_mean).pow(2) / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Type_Biased_2bit_temp   += torch.norm(emp_mean_Type_Biased_2bit - emp_mean).pow(2)   / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Type_Unbiased_2bit_temp += torch.norm(emp_mean_Type_Unbiased_2bit - emp_mean).pow(2) / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_QUICFL_1bit_temp        += torch.norm(emp_mean_QUICFL_1bit - emp_mean).pow(2)        / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_QUICFL_2bit_temp        += torch.norm(emp_mean_QUICFL_2bit - emp_mean).pow(2)        / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Kashin_1bit_temp        += torch.norm(emp_mean_Kashin_1bit - emp_mean).pow(2)        / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Kashin_2bit_temp        += torch.norm(emp_mean_Kashin_2bit - emp_mean).pow(2)        / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Scalar_1bit_temp        += torch.norm(emp_mean_Scalar_1bit - emp_mean).pow(2)        / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Scalar_2bit_temp        += torch.norm(emp_mean_Scalar_2bit - emp_mean).pow(2)        / (num_trials * sum(vec_norm_squared_list) * i)
        NMSE_Scalar_4bit_temp        += torch.norm(emp_mean_Scalar_4bit - emp_mean).pow(2)        / (num_trials * sum(vec_norm_squared_list) * i)
        #-------------------------------------
...
# Create dictionaries for average and maximum NMSE values, and save them as pickle files
nmse_avg_data = {
...
with open(os.path.join(files_dir, 'nmse_avg_data_Ber_dist.pkl'), 'wb') as avg_file:
    pickle.dump(nmse_avg_data, avg_file)

with open(os.path.join(files_dir, 'nmse_max_data_Ber_dist.pkl'), 'wb') as max_file:
